...
public class PropertyMapJSONSerializerTest {
   ...
   public void testRoundTrip() throws Exception {
      PropertyMap inner = PropertyMaps.builder().
            putString("Bar", "baz").
            ...
            putShortList("14-bit", (short) 0, (short) 16384).putIntegerList("voltage", +15, -15).
            putLongList("64-bit signed", Long.MIN_VALUE, Long.MAX_VALUE).
            putFloatList("gray", 0.5f, 0.5f, 0.5f).
            putPropertyMapList("empty pair", Arrays.asList(new PropertyMap[] {
               PropertyMaps.emptyPropertyMap(), PropertyMaps.emptyPropertyMap()
            })).
            build();
      PropertyMap pm = PropertyMaps.builder().
            putString("Foo", "foo").
            putStringList("FooBar", "foo", "bar").putInteger("answer", 42).
            putFloat("n", 1.2f).
            ...
            putAffineTransformList("ShearAndTranslate",
                  Arrays.asList(new AffineTransform[] {
   ...
   public void testLegacyAllValueTypes() throws Exception {
      ...
      data.put("test long array", new VV("Long array", new long[] { 1L + Integer.MAX_VALUE, -1L + Integer.MIN_VALUE }));
      ...
      assertEquals(Lists.newArrayList("a", "b"), pm.getStringList("test string array", new String[] {}));
      assertArrayEquals(new int[] { 1, 2, 3 }, pm.getIntegerList("test int array", new int[] {}));
      assertArrayEquals(new long[] { 1L + Integer.MAX_VALUE, -1L + Integer.MIN_VALUE }, pm.getLongList("test long array", new long[] {}));
      assertArrayEquals(new double[] { 0.25, -0.25 }, pm.getDoubleList("test double array", new double[] {}), 0.000001);
   ...
   public void testLegacySerializedObject() throws Exception {
      ...
      PropertyMap pm = PropertyMapJSONSerializer.fromJSON(String.format(
            "{" + "\"KEY\": " +
                  "{" + "\"PropType\": \"Object\", " +
                  "\"PropVal\": \"%s\"" + "}" + "}",
            base64encoded));
      assertEquals(new AffineTransform(0.0, 0.5, 0.5, 1.0, 10.0, 20.0),
